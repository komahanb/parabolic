program heat_equation_one_dim_explicit_euler
  ...
  do k = 2, N + 1 ! ignore the initial condition (marching until final time)
     ...
     write(*,'(F15.3,E15.3)'), dble(k-1)*dt, norm2(u(:,k-1) - u(:,k))/sqrt(dble(M+1))
  end do 
...
end program heat_equation_one_dim_explicit_euler
